...
The flow diagram for the Leisure node is generated below, and corresponds to **Figure 3** in the published manuscript.

```{r}
#| label: fig-hybrid-flow-diagram
#| fig-cap: Flow diagram of the Hybrid node.

plot(qgraph::flow(network$sample$graph, from = 6, theme = "colorblind"))
```
